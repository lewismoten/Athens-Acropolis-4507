...
| `DrawImageType` or similar image-placement parameter | string / enum-like text | The class strings suggest multiple image placement modes such as `IMAGE_TILE`, `IMAGE_CENTER_CENTER`, `IMAGE_LEFT_TOP`, `IMAGE_RIGHT_BOTTOM`, and similar constants. I have not confirmed the exact parameter name from my old HTML, only that the applet appears to support this idea internally. |
